...
interface DeleteCarouselDialogProps {
  open: boolean;
  mode: "single" | "bulk";
  title?: string;
  count: number;
  ...
  return (
    ...
    >
      <DialogTitle sx={{ color: "var(--text-blue-dark)", fontSize: "1rem" }}>
        {mode === "bulk"
          ? "Delete Selected Carousels"
          : "Delete Carousel Entry"}
      </DialogTitle>
      <DialogContent>
        <DialogContentText sx={{ fontSize: "0.88rem" }}>
          {mode === "bulk"
            ? `Are you sure you want to delete ${count} selected carousel${count > 1 ? "s" : ""}? This action cannot be undone.`
